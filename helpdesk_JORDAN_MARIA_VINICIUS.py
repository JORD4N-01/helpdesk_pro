...
class Chamado:
    _contador = 0
    SLA = {'critica': 4, 'alta': 8, 'media': 24, 'baixa': 72}
    TRANSICOES = {
    ...
    def __init__(self, titulo, descricao, cliente, prioridade):
        if prioridade not in self.SLA:
            raise ValueError(f"Prioridade invalida: {prioridade}")

        Chamado._contador += 1
        self.numero = Chamado._contador
        self.titulo = titulo
        ...
        self.sla_horas = self.SLA[prioridade]
        self.tecnico = None
        self.historico = []

    def __str__(self):
        horas = self.tempo_decorrido().total_seconds() / 3600
        return f"#{self.numero} - {self.titulo} ({self.cliente}) | {self.prioridade} | {self.status} | {horas:.1f}h"
    ...
    def esta_em_atraso(self):
        if self.status in ('resolvido', 'fechado'):
            return False
        return self.tempo_decorrido().total_seconds() > self.sla_horas * 3600
    ...
    def alterar_status(self, novo_status, responsavel='sistema'):
        if novo_status not in self.TRANSICOES.get(self.status, set()):
            permitidas = self.TRANSICOES[self.status]
            raise TransicaoInvalidaException(
                f"Nao pode ir de {self.status} para {novo_status}. "
                f"Transicoes: {', '.join(permitidas) if permitidas else 'nenhuma'}"
            )
...
class Tecnico:
    ...
    def __init__(self, nome, especialidades, capacidade_maxima=5):
        Tecnico._contador += 1
        self.id_tecnico = Tecnico._contador
        self.nome = nome
    ...
    def atribuir_chamado(self, numero):
        if not self.disponivel:
            raise CapacidadeExcedidaException(
                f"{self.nome} ja atingiu {self.capacidade_maxima} chamados"
            )
        self.chamados_ativos.append(numero)

    def liberar_chamado(self, numero):
        if numero not in self.chamados_ativos:
            raise ValueError(f"Chamado #{numero} nao esta com {self.nome}")
...
class CentralDeSupporte:
    ...
    def atribuir_tecnico(self, numero, id_tecnico):
        ...
        chamado.registrar_acao(f"Atribuido ao tecnico {tecnico.nome}", tecnico.nome)
    ...
    def atribuicao_automatica(self):
        ...
        while self.fila_nao_atribuidos:
            ...
            chamado.registrar_acao(
                f"Atribuido ao tecnico {tecnico.nome} (automatico)", 'sistema'
            )
            atribuidos += 1
    ...
    def resolver_chamado(self, numero, id_tecnico, descricao_solucao):
        ...
        if chamado.tecnico != id_tecnico:
            raise PermissionError(
                f"Chamado #{numero} nao esta com tecnico #{id_tecnico}"
            )
        tecnico = self.tecnicos[id_tecnico]
    ...
    def relatorio_por_prioridade(self):
        relatorio = {p: [] for p in Chamado.SLA}
    ...
    def painel_operacional(self):
        ...
        clientes_abertos = [
            c.cliente for c in self.chamados.values()
            if c.status not in ('resolvido', 'fechado')
        ]
        ...
        return {
            ...
            'tecnicos_disponiveis': [
                t.to_dict() for t in self.tecnicos.values() if t.disponivel
            ],
            'top3_clientes': [
                {'cliente': c, 'quantidade': q} for c, q in top3
            ],
        }
